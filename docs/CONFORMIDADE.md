...
| Análise com documentos só para a gestão (ADR-015) | `app/api_comum.py::verificar_gestao` — código em variável de ambiente, comparação em tempo constante, espera de 1 s a cada código errado, falha fechada sem a variável | visitantes só acessam casos fictícios; documentos reais só entram pela gestão |
